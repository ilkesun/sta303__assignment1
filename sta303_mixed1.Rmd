...
author: "Chief Science Officer Ilke Sun; ID: 1004133109"
output:
...
header-includes:    
  ...
  - \usepackage{fancyhdr}
  - \pagestyle{fancy}
  - \fancyhead[LO, LE]{Ilke Sun, 1004133109}
...
```{r}
rm(name_com_officer, names)
crew_data <- crew_data %>% 
  mutate(position = replace(position,
                            position == "Enigneering Technician",
...
week1 <- crew_data %>% 
  filter(week == 1) %>% 
  mutate(duty_shift = factor(duty_shift,
                             levels = c("Gamma", "Delta", "Beta", "Alpha")))
```
...
```{r}
w1_shift <- lm(week1$productivity ~ week1$duty_shift)
summary(w1_shift)
confint(w1_shift)
```
...
```{r}
w1_team <- lm(week1$productivity ~ week1$shift_team)
summary(w1_team)
confint(w1_team)

w1_int <- lm(week1$productivity ~ week1$duty_shift * week1$shift_team)
summary(w1_int)
confint(w1_int)

w1_full <- lm(week1$productivity ~ week1$full_team)
summary(w1_full)
confint(w1_full)

rm(week1, w1_shift, w1_team, w1_int, w1_full)
```
...
control.ops <- lmerControl(optCtrl=list(xtol_abs=1e-8, 
                         ftol_abs=1e-8, 
...
# Log diff around 1500, 1b is better 
```
...
Model 2b
$$
prod_i = a_f + a_n + (b_{n1} + b_1)week_i + b_2gpa_i + b_3score_i + e_i
$$

Model 3
$$
prod_i = a_f + a_n + (b_{f1} + b_1)week_i + b_2gpa_i + b_3score_i + e_i
$$

```{r} 
model_3 <- lmer(productivity ~ week + starfleet_gpa + perseverance_score + 
                  (1|name) + (1 + week|full_team), data = crew_data)
...
rm(model_1a, model_1b, model_2a, model_2b, model_3)
```
...
```{r}
complete_comp <- competition_data %>% 
  mutate("Vulcan orchid" = NA,
         "Pod plant" = NA,
         "Lunar flower" = NA,
         "Garlanic tree" = NA) %>% 
  select(-Klavaatu) %>% 
  merge(astrobiologists, by = "crewmember", all = TRUE)

n <- str_c(complete_comp$crewmember)
complete_comp <- as.data.frame(t(complete_comp[,-1]))
colnames(complete_comp) <- n

origin_data2 <- origin_data
origin <- origin_data2[,1]
rownames(origin_data2) <- origin
origin_data2 <- origin_data2 %>% select(-plant)

complete_comp <- complete_comp %>% 
  merge(origin_data2, by = 0, all = TRUE)

names(complete_comp)[1] <- paste("plant_names")
complete_comp <- complete_comp[-c(5, 8, 10, 11), ] 
...
rm(n, astrobiologists, origin, origin_data2, competition_data, complete_comp)
```
